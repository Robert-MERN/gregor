...
const Golfing_bays_bookings_admin = () => {
  ...
  return (
    <div className={`h-[calc(100vh-60px)] overflow-y-scroll px-[10px] lg:px-[30px] relative pt-[100px] lg:pt-[30px] pb-[40px] lg:pb-[120px] ${styles.scrollBar}`} >
      ...
      <div className='w-[90vw]  md:w-full rounded-t-md border flex items-center mb-4 overflow-hidden'>
        <button
      ...
      <div className=' bg-white w-[90vw] md:w-full flex justify-between items-center mb-6 md:mb-0' >

        <div className='w-fit z-10 relative' >
          <label
            className='text-[20px] lg:text-[28px] cursor-pointer text-stone-600 font-semibold transition-all w-fit select-none'
            htmlFor="date-picker"
          >
        ...
        <div className='flex items-end gap-14' >
          ...
          <div className='flex gap-4'>
            <button
              className={`px-[10px] lg:px-[15px] py-[4px] lg:py-[6px] text-white text-[20px] font-semibold rounded-md  grid place-items-center transition-all ${getDateWithoutTime(booking_date_for_admin) <= getDateWithoutTime(currentDate) ? "bg-stone-300" : "bg-stone-500 active:opacity-60 hover:opacity-75"}`}
            ...
            <button
              className={`px-[10px] lg:px-[15px] py-[4px] lg:py-[6px] text-white text-[20px] font-semibold rounded-md  grid place-items-center transition-all ${getDateWithoutTime(booking_date_for_admin) >= getDateWithoutTime(maxDate) ? "bg-stone-300" : "bg-stone-500 active:opacity-60 hover:opacity-75"}`}
      ...
      <div
        className=' bg-white relative w-[90vw] flex md:hidden justify-between items-center'
      >
        <div className='flex justify-between w-full gap-5' >
          <button
            onClick={() => openModal("add_edit_guests_fees_modal")}
            className='text-[13px] lg:text-[14px] font-semibold text-slate-600 font-sans border-b border-transparent hover:border-slate-300 leading-loose flex items-center justify-center gap-1 transition-all whitespace-nowrap'
          >
            <EditNoteIcon className='scale-[1.1]' />
            Add/Edit Guests's Fees
          </button>
          ...
          <button
            onClick={() => openModal("restrict_hours_modal")}
            className='text-[13px] lg:text-[14px] font-semibold text-slate-600 font-sans border-b border-transparent hover:border-slate-300 leading-loose flex items-center justify-center transition-all whitespace-nowrap'
          >
            <HourglassDisabledIcon className='scale-[.8]' />
            Restrict Hours
          </button>
      ...
      <div className='rounded-md overflow-hidden w-[90vw] md:w-full' >
        <FullCalendar
          ...
          allDaySlot={false}
          slotLabelClassNames="text-slate-500 font-semibold text-[12px] lg:text-[17px]  cursor-default font-sans"
          resourceLabelClassNames="font-medium text-[12px] lg:text-[17px]  font-sans bg-violet-500 text-white cursor-default"
          headerToolbar={{
            start: "",
            center: "",
            end: ""
